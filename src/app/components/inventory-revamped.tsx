                ...
                <div key={product.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex flex-col gap-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      />
                      {product.imageUrl ? (
                        <img src={product.imageUrl} alt={product.name} className="w-10 h-10 rounded-lg object-cover shrink-0 border border-slate-100" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                      ) : (
                        <div className="w-10 h-10 rounded-lg shrink-0 flex items-center justify-center" style={{ background: "#f0f5ff" }}>
                          <Package2 className="w-5 h-5" style={{ color: "#0a1565" }} />
                        </div>
                      )}
                      <div className="min-w-0">
                        <p className="font-semibold text-slate-900 truncate">{product.name}</p>
                        <p className="font-mono text-xs text-slate-500 truncate">{product.partNumber}</p>
                      </div>
                    </div>
                    <button onClick={() => setEditProduct(product)} className="p-1.5 rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-50">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-1">
                    <div className="bg-slate-50 p-2 rounded-lg">
                      ...
                      <p className="text-xs font-medium text-slate-700 truncate">{product.category}</p>
                    </div>
                    <div className="bg-slate-50 p-2 rounded-lg">
